...
class Registry {
   ...
   * @param {Array} args - The arguments to pass to the class constructor.
   ...
   */
  ingest(source) {
    const classes = _.merge({}, _.get(source, 'constructor.classes'), _.get(source, 'opts.classes'))
  ...
  static get(config) {
    ...
    return _.get(config, 'registry') ? config.registry : new this(config)
  }

  /**
   * Removes a specific class from the registry.
   * 
   * @param {string} name - The name of the class to be removed from the registry.
   */
  remove(name) {
    _.unset(this.classes, name)
  }
   ...
   */
  destroy() {
    for (const prop in this.classes) {
      if (this.classes.hasOwnProperty(prop)) {
        delete this.classes[prop]
      }
    }
  }
}
